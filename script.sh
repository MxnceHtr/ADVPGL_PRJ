...
PRICE=$(echo "$JSON" | grep -oP '"name":"FTSE 100","description":"FTSE 100","value":\K[0-9]+\.[0-9]+' | head -n1 | tr -d '\n')
PERCENT=$(echo "$JSON" | grep -oP '"name":"FTSE 100","description":"FTSE 100".*?"percentualchange":\K-?[0-9]+\.[0-9]+' | head -n1 | tr -d '\n')
NETCHANGE=$(echo "$JSON" | grep -oP '"name":"FTSE 100","description":"FTSE 100".*?"netchange":\K-?[0-9]+\.[0-9]+' | head -n1 | tr -d '\n')
HIGH=$(echo "$JSON" | grep -oP '"name":"FTSE 100","description":"FTSE 100".*?"high":\K[0-9]+\.[0-9]+' | head -n1 | tr -d '\n')
LOW=$(echo "$JSON" | grep -oP '"name":"FTSE 100","description":"FTSE 100".*?"low":\K[0-9]+\.[0-9]+' | head -n1 | tr -d '\n')
...
CSVFILE="./data.csv"
if [ ! -f "$CSVFILE" ]; then
    echo "Timestamp,Price,PercentChange,NetChange,High,Low,LastClose,ClosingDate,PrevClosingDate,NetChangeSign,FiftyTwoWeeksMax,FiftyTwoWeeksMin,OneYearReturn" > "$CSVFILE"
fi
